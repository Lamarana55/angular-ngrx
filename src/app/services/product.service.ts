...
export class ProductService {
  ...
  getSelectedProducts():Observable<Product[]>{
    return this.httpClient.get<Product[]>(API_URL + "/product?selected=true")
  }

  getAvailableProductus(): Observable<Product[]> {
    return this.httpClient.get<Product[]>(API_URL + "/product?available=true")
  }

  searhProducts(keword: string): Observable<Product[]> {
    return this.httpClient.get<Product[]>(API_URL + "/product?name_like=" + keword)
  }

  updateSelect(product: Product): Observable<Product> {
    product.selected = !product.selected
    return this.httpClient.put<Product>(API_URL + "/product/" + product.id, product)
  }
  ...
  deleteProducts(product: Product): Observable<any>{
    return this.httpClient.delete(API_URL + "/product/" + product.id, {responseType: 'text'})
  }
